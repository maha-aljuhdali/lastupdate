...
class LayoutCubit extends Cubit<LayoutStates> {
  ...
  void openPdf({required String link}) async {
    ...
    if( launchSuccess == false ) emit(ErrorDuringOpenPdfState(message: "حدث خطأ ما عند محاوله فتح الرابط، الرجاء المحاوله لاحقا"));
  }
  ...
  Future<void> uploadReport({required LayoutCubit layoutCubit,required String senderID,required String clubName,required String clubID,required String reportType}) async{
    ...
    if( pdfUrl != null )
      ...
      {
        emit(UploadReportToAdminWithFailureState(message: 'حدث خطأ اثناء رفع الملف الرجاء المحاوله لاحقا'));
      }
  }
  ...
  Future<void> getMyData({ClubsCubit? clubsCubit}) async {
    // TODO: ClubsCubit? clubsCubit not required عشان هباصي قيمه فقط لو جالي notification فيه ان اصبحت قائد على نادي to getDataAboutClubILead from this cubit
