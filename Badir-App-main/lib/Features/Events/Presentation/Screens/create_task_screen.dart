...
class CreateTaskScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: SafeArea(
        child: Scaffold(
          appBar: AppBar(title: const Text("إنشاء مهمه"),),
          body: Padding(
            padding: EdgeInsets.symmetric(vertical: 12.h,horizontal: 12.w),
            child: BlocConsumer<EventsCubit,EventsStates>(
              listener: (context,state)
              {
                if( state is CreateTaskLoadingState ) showLoadingDialog(context: context);
                    ...
                    DefaultButton(
                      ...
                      onTap: ()
                      {
                        if( eventsCubit.eventNameForTaskCreated != null &&_nameController.text.isNotEmpty &&_descriptionController.text.isNotEmpty &&_numOfHoursController.text.isNotEmpty &&_numOfPositionsController.text.isNotEmpty )
                        {
                          ...
                          eventsCubit.createTask(idForClubILead: idForClubILead,ownerID:Constants.userID!,taskName: _nameController.text,description: _descriptionController.text,hours: int.parse(_numOfHoursController.text.trim()),numOfPosition: int.parse(_numOfPositionsController.text.trim()),taskForPublicOrSpecificEvent: taskForPublicOrSpecificEvent);
                        }
                        else
                        {
